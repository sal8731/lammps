...
FixCrosslinked::FixCrosslinked(LAMMPS *lmp, int narg, char **arg) :
  Fix(lmp, narg, arg), nu(nullptr), N_rest(nullptr), N_0(nullptr), random(nullptr)
{
  printf("Initializing FixCrosslinked\n");
  // IS THE NUMBER OF ARGUMENTS CORRECT? //
  if (narg < 2) error->all(FLERR,"Illegal fix crosslinked command");

  // WAS NEVERY SET CORRECTLY? //
  nevery = utils::inumeric(FLERR,arg[3],false,lmp);
  if (nevery <= 0) error->all(FLERR,"Illegal fix crosslinked command");

  b = utils::numeric(FLERR,arg[4],false,lmp);
  if (b <= 0) error->all(FLERR,"Illegal fix crosslinked command - Kuhn length should be greater than 0");
  ...
  // for allocating space to array_atom for fix crosslinked
  FixCrosslinked::grow_arrays(atom->nmax);
...
void FixCrosslinked::post_constructor()
{

  // CREATE A CALL TO FIX PROPERTY/ATOM //
  new_fix_id = utils::strdup(id + std::string("_FIX_PA"));
  modify->add_fix(fmt::format("{} {} property/atom d2_nvar_{} {} ghost yes",new_fix_id, group->names[igroup],id,std::to_string(8)));
...
void FixCrosslinked::setup(int vflag)
{
  pre_force(vflag);
}
...
------------------------------------------------------------------------- */
void FixCrosslinked::pre_exchange(){
    ...
    if ((nvar[i][2] == -1 && nvar[i][0] < n_critical) || (nvar[i][3] == -1 && nvar[i][1] < n_critical)){
      ...
      for (int j = 0; j < num_bond[i]; j++){
        if (bond_type[i][j]==2){
          ...
          if (ENT_pair_tmp < 0) {
            error->one(FLERR,"Fix FixCrosslinked needs ghost atoms from further away");
          }
...
/* ---------------------------------------------------------------------- */
 ...
 void FixCrosslinked::pre_force(int vflag)
{ 
  // return;
  // Main part of code
  // ATOM COUNTS
  ...
  if (atom->nmax > nmax) {
    memory->destroy(nu);
    nmax = atom->nmax;
    memory->create(nu,nmax,2,"crosslinked:nu");
  }

  // Possibly resize arrays "N_rest" & "N_0"
  memory->destroy(N_0);
  memory->destroy(N_rest);
  memory->create(N_0,nlocal,2,"crosslinked:N_0");
  memory->create(N_rest,nlocal,2,"crosslinked:N_rest");
  ...
  for (int i = 0; i < nlocal; i++) {
    ...
    for (int jj = 0; jj < num_bond[i]; jj++){
      int i2_tag = bond_atom[i][jj];

      int i2_tmp = atom->map(i2_tag); 
      if (i2_tmp < 0) error->one(FLERR,"fix crosslinked trying to access an atom far away");
      int i2 = domain->closest_image(i,i2_tmp);

      double delx = x[i2][0] - x[i][0];
      double dely = x[i2][1] - x[i][1];
      double delz = x[i2][2] - x[i][2];

      double fbondx = 3 * delx / (100 * b * b);
      double fbondy = 3 * dely / (100 * b * b);
      double fbondz = 3 * delz / (100 * b * b);
...
void FixCrosslinked::update_nvar(tagint id1, tagint id2){
...
void FixCrosslinked::process_broken(int i, int j)
{
...
void FixCrosslinked::process_created(int i, int j, int n)
{
...
void FixCrosslinked::update_topology()
{
  ...
  for (int ilist = 0; ilist < neighbor->nlist; ilist++) {
    ...
    for (auto const &it : new_created_pairs) {
      ...
      if (i1 < 0 || i2 < 0) {
        error->one(FLERR,"Fix crosslinked needs ghost atoms "
                    "from further away");
      }
...
void FixCrosslinked::find_maxid()
{
  tagint *tag = atom->tag;
  int nlocal = atom->nlocal;
...
void FixCrosslinked::grow_arrays(int nmax)
{
  if (peratom_flag) {
    memory->grow(array_atom,nmax,size_peratom_cols,"fix_crosslinked:array_atom");
  }
}
/* ---------------------------------------------------------------------- */

void FixCrosslinked::init_myarray()
{
...
void FixCrosslinked::copy_arrays(int i, int j, int delflag)
{
  if (peratom_flag) {
    for (int m = 0; m < size_peratom_cols; m++)
...
void FixCrosslinked::set_arrays(int i)
{
  if (peratom_flag) {
    for (int m = 0; m < size_peratom_cols; m++)
      array_atom[i][m] = 0;
...
int FixCrosslinked::pack_forward_comm(int n, int *list, double *buf,
...
void FixCrosslinked::unpack_forward_comm(int n, int first, double *buf)
{
  int i,j,m,ns,last;
...
int FixCrosslinked::pack_reverse_comm(int n, int first, double *buf)
{
  int i,m,last;
...
void FixCrosslinked::unpack_reverse_comm(int n, int *list, double *buf)
{
  int i,j,m;
...
double FixCrosslinked::memory_usage()
{
  int nmax = atom->nmax;
